...
resource "aws_alb" "app-lb" {
  name               = "phonebook-lb-tf" # this name must be unique within your AWS account
...
resource "aws_launch_template" "asg-lt" {
  ...
  user_data              = base64encode(templatefile("userdata.sh", {
    ...
    user-data-git-name   = var.git_name,
    user-data-repo-name  = var.repo_name
  }))
  tag_specifications {
...
resource "aws_route53_record" "phonebook" {
  name    = "phonebook.${var.hosted_zone}"
  type    = "A"
